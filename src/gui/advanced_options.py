...
class AdvancedOptions(QMainWindow):
    def __init__(self, parent=None):
        ...
        self.le_cat_anthropic.setText(str(self.conf.categories.cat_anthropic))
    ...
    def _apply_changes(self):
        ...
        try:
            ...
            assert test_value(self, is_string_list(self.le_cat_anthropic.text()), self.le_cat_anthropic, self.scrollArea)
            for x in literal_eval(self.le_cat_anthropic.text()):
                assert test_value(self, isinstance(x, int), self.le_cat_anthropic, self.scrollArea)
            assert test_value(self, is_string_list(self.le_cat_ground.text()), self.le_cat_ground, self.scrollArea)
            for x in literal_eval(self.le_cat_ground.text()):
                assert test_value(self, isinstance(x, int), self.le_cat_ground, self.scrollArea)
        ...
        if is_string_list(self.le_cat_anthropic.text()):
            OmegaConf.update(self.conf, 'categories.cat_anthropic', literal_eval(self.le_cat_anthropic.text()))
        else:
            OmegaConf.update(self.conf, 'categories.cat_anthropic', int(self.le_cat_anthropic.text()))
