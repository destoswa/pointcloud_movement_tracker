...
def prepare_files(src_pc1, src_pc2, verbose=True):
    temp_files = []
    src_pc_out = []
    for src_pc in [src_pc1, src_pc2]:
        src_wo_ext, format = os.path.splitext(src_pc)
        format = format[1:] # remove the '.'
        if format not in ['las', 'laz']:
            if verbose:
                print(f"Converting following file into LAZ: {src_pc}")
            src_file_out = os.path.splitext(src_wo_ext)[0] + '.laz'
            ...
            src_pc_out.append(src_file_out)
            temp_files.append(src_file_out)
        else:
            src_pc_out.append(src_pc)
...
def points_in_bbox(xyz_src, xyz_tgt, node, bbox):
    ...
    pts_src = xyz_src[node.indices_src] if isinstance(node.indices_src, np.ndarray) else xyz_src
    pts_tgt = xyz_tgt[node.indices_tgt] if isinstance(node.indices_tgt, np.ndarray) else xyz_tgt
    ...
    if isinstance(node.indices_src, np.ndarray):
        return node.indices_src[mask_src], node.indices_tgt[mask_tgt], node.indices_tgt[mask_tgt_neigh]
    else:
        return np.arange(len(xyz_src))[mask_src], np.arange(len(xyz_tgt))[mask_tgt], np.arange(len(xyz_tgt))[mask_tgt_neigh]


def build_quadtree(
        xyz_src, xyz_tgt, parent, bbox, 
        indices_src, indices_tgt, indices_tgt_neigh, 
        level, min_tile_size, min_points, grid_pos, is_anthropic=False
        ):
    ...
    node = QuadNode(bbox, indices_src, indices_tgt, indices_tgt_neigh, center, level, grid_pos, parent, is_anthropic)
    ...
    if is_anthropic:
        ...
        tile_len = np.min([len(indices_src), len(indices_tgt)]) if isinstance(indices_src, np.ndarray) else np.min([xyz_src.shape[0], xyz_tgt.shape[0]])
        if tile_len < min_points:
            return node
